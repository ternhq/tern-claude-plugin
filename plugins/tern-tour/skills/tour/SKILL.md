...
`tern pr-review` needs a Tern account. Run by a person without one, it drops
...
```bash
"$TERN" pr-review "$ARGUMENTS"
```

`tern pr-review` opens the tour in the browser, then keeps the local Tern
...
```bash
"$TERN" pr-review --post-draft "$ARGUMENTS"
```
